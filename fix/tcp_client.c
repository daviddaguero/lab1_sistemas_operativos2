...
int main(int argc, char *argv[]) {
    ...
    // Determine whether the provided IP address is IPv4 or IPv6
    if (strchr(server_ip, ':') != NULL) { // Check if it's an IPv6 address
        client_socket = socket(AF_INET6, SOCK_STREAM, 0);
        if (client_socket == -1) {
            perror("Error creating IPv6 client socket");
            exit(EXIT_FAILURE);
        }
        ...
        // Connect to server using IPv6
        if (connect(client_socket, (struct sockaddr *)&server_addr6, sizeof(server_addr6)) == -1) {
        ...
        if (client_socket == -1) {
            perror("Error creating IPv4 client socket");
            exit(EXIT_FAILURE);
        }
